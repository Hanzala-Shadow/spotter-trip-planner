...
def make_schedule(legs: list[Leg], departure: datetime, cycle_used: float):
    if len(legs) != 2:
        raise ValueError(
            "The trip must contain a current-to-pickup and pickup-to-dropoff leg."
        )
    ...
    def rest(place, restart=False):
        ...
        add(
            "off_duty" if restart else "sleeper",
            "cycle_restart" if restart else "daily_rest",
            CYCLE_REST if restart else DAILY_REST,
            place,
            reason="34-hour restart restores the 70-hour cycle."
            if restart
            else "10 consecutive hours of rest restore the daily driving clocks.",
        )
    ...
    def service(kind, seconds, place):
        ...
        add(
            ...
            place,
            reason={
                "pickup": "One hour to collect the load.",
                "dropoff": "One hour to deliver the load.",
                "fuel": "30-minute fuel stop; resets distance since fueling.",
            }[kind],
        )
    ...
    for leg_index, leg in enumerate(legs):
        ...
        while elapsed < leg.seconds:
            ...
            if (
                driven >= DRIVE_LIMIT
                or (now - shift_start).total_seconds() >= SHIFT_WINDOW
            ):
                rest(place)
                continue
            ...
            seconds_to_fuel = (
                int((FUEL_MILES - fuel_distance + EPS) / speed)
                if speed > 0
            ...
            if len(events) > 2500:
                raise ValueError(
                    "This trip is too long to plan. Split it into smaller trips."
                )
    ...
    return {
        ...
        "summary": {
            "miles": total_miles,
            "driving_seconds": sum(
                e["seconds"] for e in events if e["status"] == "driving"
            ),
            "on_duty_seconds": sum(
                e["seconds"] for e in events if e["status"] in ("driving", "on_duty")
            ),
            "rest_seconds": sum(
                e["seconds"] for e in events if e["status"] in ("off_duty", "sleeper")
            ),
            "elapsed_seconds": int((now - departure).total_seconds()),
            "departure": departure.isoformat(),
            "arrival": now.isoformat(),
            "cycle_used_at_finish": cycle / HOUR,
    ...
    while midnight < trip_end:
            ...
            segments.append(
                {
                    ...
                    "miles": event["miles"] * duration / event["seconds"],
                    "event_id": event["id"],
                }
            )
            remarks.append(
                {
                    ...
                    if a == start
                    else "Continued from previous log",
                    "reason": event["reason"],
                }
            )
        if segments[0]["start_second"] > 0:
            segments.insert(
                ...
                {
                    "status": "off_duty",
                    "kind": "assumed_off_duty",
                    "start_second": 0,
                    "end_second": segments[0]["start_second"],
                    "miles": 0,
                    "event_id": None,
                },
            )
            remarks.insert(
                0,
                {
                    "time": "00:00",
                    "kind": "assumed_off_duty",
                    "location": events[0]["start_place"]["label"],
                    "reason": "Assumed off duty before this planned trip.",
                },
            )
        if segments[-1]["end_second"] < 86400:
            segments.append(
                {
                    "status": "off_duty",
                    "kind": "assumed_off_duty",
                    "start_second": segments[-1]["end_second"],
                    "end_second": 86400,
                    "miles": 0,
                    "event_id": None,
                }
            ...
            remarks.append(
                {
                    "time": trip_end.strftime("%H:%M"),
                    "kind": "assumed_off_duty",
                    "location": events[-1]["end_place"]["label"],
                    "reason": "Assumed off duty after this planned trip.",
                }
            )
        totals = {
            status: sum(
