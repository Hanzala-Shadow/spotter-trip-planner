...
TRIP = {"current":{"label":"Chicago, IL","lon":-87.6298,"lat":41.8781},
...
def test_api_integrates_route_and_schedule(client):
    ...
    assert [p.label for p in fetch.call_args[0][0]] == ['Chicago, IL','Springfield, IL','Nashville, TN']


@pytest.mark.parametrize('value',[-1,71,True,'10',None,float('nan'),float('inf')])
def test_invalid_cycle_input(client,value):
    data={**TRIP,'cycle_used':value}
    assert client.post('/api/plan',data,content_type='application/json').status_code == 400


@pytest.mark.parametrize('key,value',[('current',None),('pickup',{'label':'x'}),('departure','not a date'),('departure','2026-01-01T00:00+05:00'),('utc_offset_minutes',0),('driver_details',[])])
def test_invalid_fields(client,key,value):
    assert client.post('/api/plan',{**TRIP,key:value},content_type='application/json').status_code == 400


def test_outside_supported_area(client):
    data=copy.deepcopy(TRIP);data['current']['lon']=73
    assert client.post('/api/plan',data,content_type='application/json').status_code == 400


def test_invalid_json_and_content_type(client):
    assert client.post('/api/plan','{',content_type='application/json').status_code == 400
    assert client.post('/api/plan',json.dumps(TRIP),content_type='text/plain').status_code == 415
    assert client.get('/api/plan').status_code == 405


def test_provider_failure_never_fabricates_results(client):
    with patch('planner.providers.fetch_route',side_effect=ProviderError('Route unavailable.')):
        response=client.post('/api/plan',TRIP,content_type='application/json')
...
def test_location_failure(client):
    with patch('planner.providers.search_places',side_effect=ProviderError('Unavailable')):
        assert client.get('/api/locations?q=Chicago').status_code == 503


def test_large_payload_is_rejected(client):
    response=client.post('/api/plan',json.dumps({'x':'a'*40000}),content_type='application/json')
