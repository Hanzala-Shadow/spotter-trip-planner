...
def test_vercel_wsgi_exports(endpoint, method, body, status):
    ...
    environ = {
        ...
        "PATH_INFO": "/provider-internal-path",
        "QUERY_STRING": "q=Chicago",
    ...
    headers = []
    with (
        patch("planner.providers.fetch_route", return_value=route(4, 8)),
        patch("planner.providers.enrich_stop_labels"),
        patch("planner.providers.search_places", return_value=[TRIP["current"]]),
    ):
