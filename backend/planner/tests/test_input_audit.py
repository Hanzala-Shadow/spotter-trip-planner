...
@pytest.mark.parametrize(
    "field", ["cycle_used", "utc_offset_minutes", "longitude", "latitude"]
)
...
@pytest.mark.parametrize(
    "departure", ["2026-09-15", "20260915", "2026-09-15T06", "2026-09-15X06:00"]
)
def test_departure_requires_an_explicit_date_and_time(client, departure):
    with (
        patch.object(providers, "fetch_route", return_value=route(1, 1)),
        patch.object(providers, "enrich_stop_labels"),
    ):
...
@pytest.mark.parametrize(
    "payload", [{}, {"features": None}, {"features": {}}, {"features": "unavailable"}]
)
...
def test_search_ignores_invalid_coordinates(coordinates):
    invalid = {
        "properties": {"name": "Invalid", "countrycode": "US"},
        "geometry": {"coordinates": coordinates},
    }
    valid = {
        "properties": {"name": "Chicago", "state": "Illinois", "countrycode": "US"},
        "geometry": {"coordinates": [-87, 41]},
    }
    with patch.object(
        providers, "get_json", return_value={"features": [None, invalid, valid]}
    ):
...
def test_invalid_route_payloads_become_503(client, case):
    ...
    if case == "route_null":
        ...
        payload["routes"][0]["legs"] = None
    elif case == "step_null":
        leg["steps"] = [None]
    elif case == "maneuver_null":
        step["maneuver"] = None
    elif case == "geometry_null":
        step["geometry"] = None
    elif case == "nan_point":
        step["geometry"]["coordinates"][0] = [float("nan"), 41]
    elif case == "short_point":
        step["geometry"]["coordinates"][0] = [-87]
    elif case == "huge_duration":
        leg["duration"] = 10**400
    elif case == "negative_step":
        step["distance"] = -1
    elif case == "nan_step":
        step["distance"] = float("nan")
    with (
        patch.object(providers, "get_json", return_value=payload),
        patch.object(providers, "enrich_stop_labels"),
    ):
...
@pytest.mark.parametrize(
    "payload", ["null", "[]", "true", '"trip"', "{", '{"cycle_used":NaN}']
)
def test_invalid_json_shapes_are_400(client, payload):
    assert (
        client.post("/api/plan", payload, content_type="application/json").status_code
        == 400
    )


@pytest.mark.parametrize(
    "field", ["driver", "carrier", "vehicle", "shipping", "office"]
)
def test_optional_text_limit_is_enforced(client, field):
    response = client.post(
        "/api/plan",
        {**TRIP, "driver_details": {field: "x" * 151}},
        content_type="application/json",
    )
    assert response.status_code == 400
