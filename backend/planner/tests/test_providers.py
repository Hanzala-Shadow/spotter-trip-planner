...
def test_no_route_is_reported():
    with patch.object(providers,'get_json',return_value={'code':'NoRoute'}):
        with pytest.raises(providers.ProviderError):providers.fetch_route([A,B,C])


def test_empty_geometry_on_nonzero_trip_is_rejected():
    data={'code':'Ok','routes':[{'legs':[{'distance':1000,'duration':60,'steps':[]}]*2}]}
    with patch.object(providers,'get_json',return_value=data):
        with pytest.raises(providers.ProviderError):providers.fetch_route([A,B,C])


def test_duration_has_documented_speed_cap():
    raw={'distance':160934.4,'duration':3600,'steps':[{'geometry':{'coordinates':[[A.lon,A.lat],[B.lon,B.lat]]},'maneuver':{'type':'depart'},'distance':160934.4,'name':'I-55'}]}
    with patch.object(providers,'get_json',return_value={'code':'Ok','routes':[{'legs':[raw,raw]}]}):
...
def test_request_timeout_becomes_provider_error():
    with patch.object(providers.httpx,'get',side_effect=httpx.ReadTimeout('timed out')):
        with pytest.raises(providers.ProviderError):providers.get_json('https://example.test/timeout',{})


def test_search_filters_non_us_locations():
    features=[{'properties':{'name':'Paris','countrycode':'FR'},'geometry':{'coordinates':[2,48]}},
              {'properties':{'name':'Chicago','city':'Chicago','state':'Illinois','countrycode':'US'},'geometry':{'coordinates':[-87,41]}}]
...
def test_reverse_lookup_is_approximate_city_state():
    data={'features':[{'properties':{'city':'Flagstaff','state':'Arizona','countrycode':'US'}}]}
    with patch.object(providers,'get_json',return_value=data):
        label=providers.nearby_label({'lon':-111.65,'lat':35.2,'label':'Along route'})
...
def test_enrichment_changes_names_without_moving_events():
    ...
    events=make_schedule(route(4,20),datetime(2026,9,15,tzinfo=timezone.utc),0)['events']
    original=deepcopy(events)
    with patch.object(providers,'nearby_label',return_value='Near Test City, Arizona') as lookup:
