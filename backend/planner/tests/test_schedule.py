...
def route(hours1=4, hours2=6, miles1=None, miles2=None):
    return [Leg(A, B, hours1*50 if miles1 is None else miles1, ceil(hours1*3600), [[A.lon,A.lat],[B.lon,B.lat]]),
            Leg(B, C, hours2*50 if miles2 is None else miles2, ceil(hours2*3600), [[B.lon,B.lat],[C.lon,C.lat]])]
...
def check_rules(result, initial_cycle):
    ...
    for i, event in enumerate(events):
        ...
        if i: assert events[i-1]["end"] == event["start"]
        if event["kind"] in ("daily_rest", "cycle_restart"):
            ...
            if event["status"] == "on_duty": cycle += event["seconds"]
            if event["seconds"] >= 1800: since_break = 0
            if event["kind"] == "fuel": fuel_miles = 0
    for log in result["logs"]:
        assert sum(log["totals"].values()) == 86400
        assert log["segments"][0]["start_second"] == 0
        assert log["segments"][-1]["end_second"] == 86400
        for a,b in zip(log["segments"],log["segments"][1:]): assert a["end_second"] == b["start_second"]
    assert sum(log["miles"] for log in result["logs"]) == pytest.approx(result["summary"]["miles"])
...
def test_short_trip_has_exact_service_and_mileage():
    ...
    assert [e["kind"] for e in result["events"]] == ["drive","pickup","drive","dropoff"]
...
def test_midnight_does_not_restore_driving_hours():
    ...
    assert sum(e["seconds"] for e in before_rest if e["status"] == "driving") == 11*3600
    check_rules(result, 0)


@pytest.mark.parametrize("cycle", [0, .125, 60, 69, 69.999, 70])
def test_cycle_boundaries(cycle):
    result = make_schedule(route(12,15), START, cycle)
    check_rules(result, cycle)
    if cycle == 70: assert result["events"][0]["kind"] == "cycle_restart"
...
@given(st.integers(0,12000), st.integers(0,12000), st.integers(0,70000), st.integers(0,23))
def test_generated_trips_preserve_all_rules(minutes1,minutes2,cycle_thousandths,hour):
    cycle = cycle_thousandths/1000
    result = make_schedule(route(minutes1/60,minutes2/60), START.replace(hour=hour), cycle)
    check_rules(result, cycle)


@pytest.mark.parametrize("cycle", [-1,71])
def test_invalid_cycle_rejected(cycle):
    with pytest.raises(ValueError): make_schedule(route(), START, cycle)
