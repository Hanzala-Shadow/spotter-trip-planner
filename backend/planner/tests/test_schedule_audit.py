...
def interval_audit(result, initial_cycle, expected_miles, expected_drive):
    ...
    def clock_start(allowed, minimum, before):
        valid = [
            b for a, b in blocks(allowed, before) if (b - a).total_seconds() >= minimum
        ]
        return valid[-1] if valid else beginning

    def seconds_between(start, end, allowed):
        return sum(
            max(0, (min(b, end) - max(a, start)).total_seconds())
            for a, b, status in spans
            if status in allowed
        )
    ...
    for a, b, status in spans:
        ...
        work_start = min(
            x for x, _, s in spans if x >= daily and s in {"driving", "on_duty"}
        )
        ...
        assert (
            prior + seconds_between(cycle_start, b, {"driving", "on_duty"}) <= 70 * 3600
        )
    ...
    assert (
        ...
        == (spans[-1][1] - beginning).total_seconds()
    )
    assert result["summary"]["on_duty_seconds"] == seconds_between(
        beginning, spans[-1][1], {"on_duty", "driving"}
    )
    positions = (
        [0.0]
        + [e["route_miles"] for e in events if e["kind"] == "fuel"]
        + [expected_miles]
    )
    ...
    for kind in ("pickup", "dropoff"):
        ...
        assert (
            len(service) == 1
            and service[0]["seconds"] == 3600
            and service[0]["status"] == "on_duty"
        )
    ...
    for log in result["logs"]:
        ...
        assert all(
            a["end_second"] == b["start_second"] for a, b in zip(segments, segments[1:])
        )
        ...
        assert log["totals"]["off_duty"] == 86400 - sum(
            log["totals"][s] for s in ("driving", "on_duty", "sleeper")
        )
        assert all(
            int(r["time"][:2]) < 24 and int(r["time"][3:]) < 60 for r in log["remarks"]
        )
...
def trip(seconds1, seconds2, miles1=None, miles2=None):
    return [
        Leg(
            a,
            b,
            seconds / 3600 * 55 if miles is None else miles,
            seconds,
...
@given(
    seconds1=st.integers(0, 100 * 3600),
    seconds2=st.integers(0, 100 * 3600),
    speed1=st.integers(1, 55),
    speed2=st.integers(1, 55),
    cycle_seconds=st.integers(0, 70 * 3600),
    minute=st.integers(0, 1439),
    offset=st.sampled_from([-480, -420, -360, -300, -240]),
)
def test_generated_schedules_against_independent_intervals(
    seconds1, seconds2, speed1, speed2, cycle_seconds, minute, offset
):
    departure = datetime(
        2026,
        9,
    ...
    interval_audit(
        result, cycle_seconds / 3600, sum(l.miles for l in legs), seconds1 + seconds2
    )
