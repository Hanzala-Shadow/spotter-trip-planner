...
function validatePlan(plan) {
  ...
  for (const log of plan.logs) {
    assert.equal(Object.values(log.totals).reduce((a, b) => a + b, 0), 86400);
  }
  ...
  const fuelPositions = [0, ...plan.events.filter(event => event.kind === 'fuel')
    .map(event => event.route_miles), plan.summary.miles];
...
async function generate(page, sample) {
  ...
  const response = page.waitForResponse(response =>
    response.url().endsWith('/api/plan') && response.request().method() === 'POST',
  { timeout: 60000 });
  ...
  const evidence = {
    sample, request_seconds: (Date.now() - started) / 1000,
    summary: plan.summary,
    logs: plan.logs.map(log => ({ date: log.date, miles: log.miles, totals: log.totals })),
    events: plan.events.map(event => ({
      kind: event.kind, start: event.start, end: event.end, seconds: event.seconds,
      miles: event.miles, route_miles: event.route_miles, location: event.start_place.label,
    })),
  };
...
try {
  ...
  for (const [label, value] of Object.entries({
    'Driver name': 'Demo Driver', Carrier: 'Demo Carrier',
    'Vehicle / trailer': 'Truck 101 / Trailer 201',
    'Shipping document': 'DEMO-001', 'Office / terminal': 'Demo Terminal',
  })) {
  ...
  await expect(page.getByRole('heading', { name: 'Los Angeles to Dallas', exact: true }))
    .toBeVisible();
  ...
  for (let index = 0; index < main.logs.length; index++) {
    await expect(page.getByRole('tabpanel').getByLabel('Daily log for ' + main.logs[index].date))
      .toBeVisible();
